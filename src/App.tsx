...
function ExerciseVideoGuide({workout}:{workout:Workout}){return <details className="exercise-video-guide" open><summary><span><b>Exercise video guide</b><small>YouTube Shorts and TikTok form searches</small></span><strong>{workout.exercises.length} exercises</strong></summary><div className="video-guide-list">{workout.exercises.map((exercise,index)=><div key={exercise.id}><span className="sequence">{index+1}</span><b>{exercise.name}</b><span className="video-guide-actions"><a href={exerciseVideoSearch('youtube',exercise.name)} target="_blank" rel="noreferrer">YouTube Shorts ↗</a><a href={exerciseVideoSearch('tiktok',exercise.name)} target="_blank" rel="noreferrer">TikTok ↗</a></span></div>)}</div><p>Check that the demonstration matches your available machine and comfortable range of motion.</p></details>}
...
function WorkoutView({date,workout,store,setStore,onDone}:{date:string;workout:Workout;store:Store;setStore:(s:Store)=>void;onDone:()=>void}){
  ...
  return <div className="page workout-page">{date!==easternDate()&&<p className="past-session-note">Editing workout for <b>{dayNames[workout.day]}, {formatDate(date)}</b>. Saving will update that day's existing session without creating a duplicate.</p>}<section className="workout-banner"><div><span className="eyebrow">{dayNames[workout.day]} · {existing?'EDIT SESSION':'WORKOUT SESSION'}</span><h2>{workout.title}</h2><p>{workout.focus}</p></div><div className="session-progress"><b>{completed}/{total}</b><small>sets complete</small></div></section>
    ...
    <ExerciseVideoGuide workout={workout}/>
